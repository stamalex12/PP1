                ...
                <table class="table table-striped">
                    ...
                    @foreach($roombooking as $booking)
                        <tr>
                            ...
                            <td><a href="{{ url('admin/roombooking/' . $booking->id) }}">{{ $booking->id }}</a></td>
                            <td>{{ $booking->roomId }}</td>
                            <td>{{ date("g:ia\, jS M Y", strtotime($booking->startDate)) }}</td>
                            <td>{{date("g:ia\, jS M Y", strtotime($booking->endDate)) }}</td>
                            <td>
                                <a class="btn btn-primary btn-xs" href="{{ url('admin/roombooking/' . $booking->id . '/edit')}}">
                                    <span class="glyphicon glyphicon-edit"></span> Edit</a>
                                <form action="{{ url('admin/roombooking/' . $booking->id) }}" style="display:inline" method="POST">
